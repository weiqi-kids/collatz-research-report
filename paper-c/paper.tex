...
$B/T$ over the longest descents starting from these integers takes
values in the two-element set $\{65/41,\, 149/94\}$ -- the two
``upper'' continued-fraction objects for $\log_2 3$ with denominator
at most $94$ -- and in particular satisfies $B/T \geq 149/94$ in
every length-bin tested; we record this as
Conjecture~\ref{conj:65-41}.  As a rigorous corollary
(Corollary~\ref{cor:cycle}) we use the irrationality of $\log_2 3$
to derive the cycle-element upper bound
...
phenomenon: across all bit-shells $9 \leq k \leq 44$ that we tested
(with $\Omega_k = \Nodd \cap [2^{k-1},\, 2^k)$), the worst-case
descent ratio $B/T$ over first-descent orbits originating in
$\Omega_k$ takes values only in the two-element set
$\{65/41,\, 149/94\}$: it equals $65/41$ in essentially every
bit-shell, with rare excursions to $149/94$ at $k \in \{19, 29, 31\}$.
Both ratios are intimately tied to the continued-fraction expansion:
$65/41$ is the sixth principal convergent of $\log_2 3$, and $149/94$
is the next best ``above'' semiconvergent.
...
\begin{enumerate}[label=(\roman*),leftmargin=2em]
  ...
  \item \emph{Empirical $149/94$ conjecture}
    (Conjecture~\ref{conj:65-41}, computational).
    For every odd integer $n$ in the search range
    $\{3, 5, \ldots, 2^{44} - 1\}$ for which a length-$T$ first
    descent of $S$ has been computed, the ratio $B_T/T$ satisfies
    $B_T / T \geq 149/94$, and the realized worst-case value of
    $B_T/T$ in each bit-bin lies in the two-element set
    $\{65/41,\, 149/94\}$, with the rare excursions to $149/94$
    occurring at $k \in \{19, 29, 31\}$ in our sample.
  \item \emph{Cycle-element upper bound}
    ...
    $2^B - 3^P \geq 1$ by unique factorization (the irrationality of
    $\log_2 3$).
\end{enumerate}
...
$B(T)/T > 65/41$ strictly: no fraction $B/T$ with such $T$ and
$\log_2 3 < B/T \leq 65/41$ can exist, because the next record-improver
after $T = 41$ has denominator $94 > 93$.  Equality $B(T)/T = 65/41$
...
\midrule
$5$  & $8$        & $27$    & $59/37$  & non-canonical (small-$T$ regime) \\
$6$  & $16$       & $63$    & $54/34 = 27/17$ & upper semiconvergent below $h_3$ \\
$7$  & $32$       & $97$    & $27/17$  & upper semiconvergent \\
$8$  & $64$       & $255$   & $46/29$  & upper semiconvergent \\
$9$  & $128$      & $447$   & $65/41$  & $h_5$ \\
$10$ & $256$      & $703$   & $65/41$  & $h_5$ \\
$11$ & $512$      & $1639$  & $65/41$  & $h_5$ \\
$12$ & $1024$     & ---     & $65/41$  & $h_5$ \\
$13$ & $2048$     & $4591$  & $65/41$  & $h_5$ \\
$14$ & $4096$     & $8255$  & $27/17$  & upper semiconvergent (anomaly, see text) \\
$15$ & $8192$     & $25295$ & $65/41$  & $h_5$ \\
$16$ & $16384$    & $52527$ & $65/41$  & $h_5$ \\
$17$ & $32768$    & $93823$ & $65/41$  & $h_5$ \\
$18$ & $65536$    & $154079$ & $65/41$ & $h_5$ \\
\bottomrule
\end{tabular}
\caption{Experiment E1: exhaustive worst-case descent ratios
$\rho^{*}(k)$ over the bit-shell
$\Omega_k = \Nodd \cap [2^{k-1},\, 2^k)$ for $5 \leq k \leq 18$.
``$h_5$'' denotes the sixth principal convergent $65/41$ of
$\log_2 3$; ``upper semiconvergent'' denotes a best-above
approximation between two principal convergents.  Note that, because
$\Omega_k$ are disjoint shells, $\rho^{*}(k)$ is not constrained to
be monotone in $k$: the $k = 14$ entry, for instance, is an
``anomalously high'' value $27/17$, arising because the worst
length-$17$ first descent in $[2^{13},\,2^{14})$ realizes $27/17$
rather than $65/41$ (no orbit in this shell has a length-$41$ first
descent attaining the convergent value).  The phenomenon disappears
once $k$ grows enough to admit length-$29$ and length-$41$ first
descents.}
\label{tab:E1}
\end{table}
...
For every odd integer $n \geq 3$, the first-descent ratio of $n$
under the compressed Collatz map satisfies
...
over $n$ in any bit-bin lies in the two-element set
$\{65/41,\, 149/94\}$, the only two upper rational approximations to
$\log_2 3$ with denominator at most $94$ and value strictly less than
$1.59$.  Equivalently, every length-$T$ initial segment of a
compressed Collatz orbit at which a first descent occurs satisfies
$B_T/T \geq 149/94$.
\end{conjecture}
...
establishes the deterministic bound $B(T)/T \geq 65/41$ only for
$T \leq 93$.  At $T = 94$ the deterministic minimum already drops to
$149/94$, which is precisely realized by some orbits in our data
(see Table~\ref{tab:E3}).  The conjecture asserts that beyond
$T = 94$, the deterministic floor is \emph{not} further explored by
actual Collatz orbits: no first-descent segment in the sampled range
has $\rho_{\mathrm{des}} \in (\log_2 3,\, 149/94)$, even though the
next deterministic above-fraction is $233/147 < 149/94$.  The
phenomenon thus combines a deterministic continued-fraction bound
(tight at $T = 41$) with a separate empirical phenomenon (orbits
realize only the first two upper continued-fraction objects, $65/41$
and $149/94$, as worst-case first-descent ratios at the sampled
scales).
\end{remark}
...
\begin{observation}[Worst-case orbit shape, $k \geq 9$]\label{obs:shape}
For every bit-bin $k$ with $9 \leq k \leq 44$ in Section~\ref{sec:exp},
the length of first descent $T_{\mathrm{des}}$ realizing
$\rho^{*}_{\mathrm{obs}}(k)$ takes one of the values
$T_{\mathrm{des}} \in \{41, 82, 94\}$: at $T = 41$ and $T = 82$ the
realized ratio is $65/41$, at $T = 94$ it is $149/94$.  No worst-case
orbit identified in this range had $T_{\mathrm{des}}$ strictly between
$41$ and $94$ apart from $82$, or strictly greater than $94$.  We
emphasize that this observation is restricted to $k \geq 9$:
small-$T$ regimes ($k \leq 8$, see Table~\ref{tab:E1}) realize
worst-case denominators $T_{\mathrm{des}} \in \{17, 29, 37\}$ from
upper semiconvergents below $h_5$.
\end{observation}
...
(Lemma~\ref{lem:cycleeq}) combined with the irrationality of
$\log_2 3$ (Lemma~\ref{lem:gs}), which we record because the same
machinery is
